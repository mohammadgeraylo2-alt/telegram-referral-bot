...
# کاربرهایی که دکمه‌ی «فالو پیج اینستاگرام» رو زدن (افتخاری - بدون چک واقعی)
ig_followed_users: set[int] = set()

# لینک پیج اینستاگرام - اگه توی config.py متغیر INSTAGRAM_PAGE_URL تعریف نشده باشه از این استفاده میشه
INSTAGRAM_PAGE_URL = getattr(config, "INSTAGRAM_PAGE_URL", "https://instagram.com/instasavexx")
...
def build_join_keyboard(not_joined, payload: str = "", need_instagram: bool = False):
    rows = []
    for username, title in not_joined:
        rows.append([InlineKeyboardButton(f"عضویت در {title}", url=f"https://t.me/{username}")])
    if need_instagram:
        rows.append([InlineKeyboardButton("📸 فالو پیج اینستاگرام", url=INSTAGRAM_PAGE_URL)])
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    need_ig = user.id not in ig_followed_users
    if not_joined or need_ig:
        await update.message.reply_text(
            f"{config.BOT_TITLE}\n\n"
            "برای استفاده از ربات، اول باید توی کانال‌های زیر عضو بشی و پیج اینستاگرامو فالو کنی 👇",
            reply_markup=build_join_keyboard(not_joined, payload, need_ig),
        )
        return
...
async def check_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    not_joined = await get_not_joined_channels(context, user_id)

    if not_joined:
        await query.answer("هنوز توی همه کانال‌ها عضو نشدی!", show_alert=True)
        return

    ig_followed_users.add(user_id)  # افتخاری: با همین کلیک تایید میشه
